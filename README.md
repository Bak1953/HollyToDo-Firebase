...
- Optional push notification reminders one day before a task is due
...
6. In Project settings → Cloud Messaging, copy the **Web Push certificate (VAPID key)** and paste it into `script.js` where it says `YOUR_VAPID_KEY_HERE`.

## Deploy the web app

You can host the front end on **Firebase Hosting** or **GitHub Pages**:

### GitHub Pages

1. Create a GitHub repository, e.g. `Bak1953/HollyToDo-Firebase`.
2. Push this folder to the `main` branch.
3. In the repository settings, enable GitHub Pages from the `main` branch.
4. Add the hosted URL to Firebase Console → Authentication → Authorized domains.
5. Add the same URL to Firebase Console → Project settings → Add app check or restricted API keys if you use them.
...
1. Install the Firebase CLI:
   ...
   ```
2. Login and initialize hosting in this folder:
   ```bash
   firebase login
...
1. In the Firebase console, upgrade to the **Blaze** plan (Cloud Functions require billing).
2. In the `functions` folder, install dependencies:
   ```bash
   cd functions
   npm install
   ```
3. Deploy the function and Firestore rules:
   ```bash
...
- `firebase-messaging-sw.js` must be at the root of the hosted site for FCM to work.
